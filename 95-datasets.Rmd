...
:   In the first chapters we want to set up a working classifier as
    ...
    Then we perform a 60/20/20% split. The rows in the data frame
    are already randomised, therefore the first 300 can constitute
    the training sample, the next 100 can define the validation sample,
    and the 100 remaining ones -- the test one.

    ```{r wine-train-test-split4,eval=FALSE}
    write.csv(wine_subset[1:300, ],
        "datasets/wine_train.csv", row.names=FALSE)
    write.csv(wine_subset[301:400, ],
        "datasets/wine_validate.csv", row.names=FALSE)
    write.csv(wine_subset[401:500, ],
