...
rates <- read.csv("datasets/currency_exchange_diff.csv.gz",
    comment.char="#", stringsAsFactors=FALSE)
head(rates[,1:8]) # few first rows and columns
tail(rates[,1:8]) # a couple of last rows and a few columns
```
...
data frame `rates_train` that consists of the *first* 80\% rows
in `rates` and a data frame `rates_test` that includes the remaining 20\%.
{ END exercise }
