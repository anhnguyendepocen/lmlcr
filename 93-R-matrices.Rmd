...
Let's load the dataset on historical daily EUR to AUD/NZD/GBP/PLN/etc.
currency exchange rates:
...
{ BEGIN exercise }
Draw a plot (see `?plot`) of EUR/AUD
...
{ BEGIN exercise }
Draw a plot of the NZD/AUD exchange rates
(by appropriately transforming EUR/AUD and EUD/NZD columns).
{ END exercise }
...
:   To do the above, for each currency separately, divide
    every exchange rate by the rate found the first row.
    For example, relative rate  of EUR/GBP equal to
    ca. `r round(X[nrow(X),"GBP"]/X[1,"GBP"], 2)`
    on the last day means that you could buy, with the same amount of EUR,
    `r round(100*X[nrow(X),"GBP"]/X[1,"GBP"])-100`% more of pounds
    on 2020-05-19
    (where 1 EUR = `r X[nrow(X),"GBP"]` GBP)
    than on 1999-01-04
    (where 1 EUR = `r X[1,"GBP"]` GBP).
...
{ BEGIN exercise }
Explain in your own words (consider different scenarios):

* If you were a German investor in Scotland
would it be better to have the relative EUR/GBP exchange rates going up or down?
* What if  you were a Welsh tourist in Estonia?
* What if you were a producer of goods made from imported as well as
locally sourced parts/ingredients?
{ END exercise }
...
```{r matrix_ex2d,echo=FALSE,fig.height=5,fig.cap="Currency exchange rates relative to 1999-01-04 (with EUR as the base currency)"}
matplot(rev(rates[,1]), Y, xlab="Day", ylab="Relative Exchange Rate",
    type="l")
legend("topleft", legend=paste0("EUR/", dimnames(Y)[[2]]), col=1:(ncol(Y)), lty=1:(ncol(Y)),
bg="white")
