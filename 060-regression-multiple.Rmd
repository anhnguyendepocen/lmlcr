...
# Multiple Regression  {#chap:regression-multiple}
...
```{r simple_recap2,fig.cap="Fitted regression line for the Credit dataset"}
...
```{r x12_y,echo=FALSE,fig.cap="Scatter plots of $Y$ vs. $X_1$ and $X_2$"}
par(mfrow=c(1,2))
...
```{r  x12_ycolmap,echo=FALSE,fig.cap="A heatmap for Rating as a function of Balance and Income; greens represent low credit ratings, whereas reds -- high ones"}
...
:   (\*) You may ask why don't we fit models so as to minimise the MAE
    ...
    enough to know about the mean absolute error regression task). However, it may be worth
    trying it out sometimes.

    Sometimes we might prefer MAE regression to the classic one
...
#### Graphical Summaries of Residuals {#sec:graphicalsummaryresidual}
...
* Outliers == observations potentially worth inspecting (is it a bug or a feature?) -- values < Q1 - 1.5IQR or > Q3 + 1.5 IQR


```{r boxplot_explained,echo=FALSE,fig.cap="An example boxplot"}
x <- f1$residuals
...
```{r boxplot_residuals,fig.cap="Box plots of the residuals for the three models studied"}
...
```{r violinplot_residuals,message=FALSE,fig.cap="Violin plots of the residuals for the three models studied"}
library("vioplot")
...
```{r absresiduals_boxplot,fig.cap="Box plots of the modules of the residuals for the three models studied"}
...
```{r resid_vs_fitted,fig.cap="Residuals vs. fitted outputs for the three regression models"}
...
However, in the `Credit` data  set there are other variables
...
```{r pearson_interpret,echo=FALSE,fig.height=6,fig.cap="Different datasets and the corresponding Pearson's $r$ coefficients"}
par(mfrow=c(2,2))
set.seed(123)
x <- runif(25)
...
From Figure \@ref(fig:poly2) we see that the models tend to agree on\
what the predictions should be for low and moderate balances $(X_1)$.
However, there's clearly a problem
with the behaviour of the degree-10 polynomial on the righthand side of the figure.
...
Let's consider the World Factbook 2020 dataset (see Appendix F<!-- TODO -->),
which lists country names, their population,
area, GDP, mortality rates and so forth:
...
<!--
 Omitting of the aforementioned  columns can be done in a few ways. Here is one:
...
```{r factbookB15, fig.cap="Most correlated pair of variables and the fitted regression line"}
...
See Figure \@ref(fig:factbookB15) for the result.
...
```{r factbookB13, fig.cap="A bugfixed version of Figure \\@ref(fig:factbookB9)"}
#C <- data.frame(
...
Let's play with World Factbook 2020 once again.
...
### Median House Value in Boston (Continued)


Let's get back to the `Boston` dataset from the `MASS` package
that we considered in the chapter on simple linear regression:

```{r Boston_load2,message=FALSE}
library("MASS")
head(Boston, 3)
```
...
{ BEGIN exercise }
Construct a multiple regression model for `medv` as a function
of `lstat`, `rm` and `tax`.
{ END exercise }

{ BEGIN exercise }
Use forward selection (with respect to the AIC criterion)
to come up with a multiple regression model
for `medv` as a function of other variables.
{ END exercise }

{ BEGIN exercise }
Use backward elimination  (with respect to the AIC criterion) to
construct a multiple regression model for `medv` as a function
of other variables.
{ END exercise }
...
{ BEGIN exercise }
Construct a multiple regression model for `medv` as a function
of `lstat`, `rm` and `tax` transformed in various ways.
Apply logarithms, squares, exponential (amongst others)
functions on the variables
and try to come up with the best model by trial and error
(you can also use forward selection and/or backward elimination
for this).
{ END exercise }
...
{ BEGIN exercise }
Compare the four models in terms of  AIC, RMSE, MAE and adjusted $R^2$.
Which model is the best with regards to each metric?
